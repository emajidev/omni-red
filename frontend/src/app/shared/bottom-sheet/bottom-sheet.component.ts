...
@Component({
  ...
  template: `
    <!-- Backdrop -->
    <div class="sheet-backdrop fixed inset-0 z-[1000] backdrop-blur-sm transition-opacity"
         (click)="close.emit()"></div>

    <!-- Sheet -->
    <section
      class="sheet-panel fixed bottom-0 inset-x-0 sm:left-[10vw] sm:right-[10vw] sm:w-[80vw] z-[1001] flex max-h-[85vh] sm:max-h-[75vh] min-h-[50vh] sm:min-h-[45vh] flex-col
             rounded-t-3xl shadow-sheet animate-sheet-up
             pb-[env(safe-area-inset-bottom)]"
      role="dialog" aria-modal="true">

      <!-- Grab handle -->
      <div class="flex justify-center pt-3 pb-1">
        <span class="sheet-grab h-1.5 w-12 rounded-full"></span>
      </div>

      <!-- Header (Optional) -->
      @if (!hideHeader) {
        <header class="flex items-center gap-3 px-5 pt-2 pb-4 border-b" style="border-color: var(--divider)">
          <span class="grid h-10 w-10 place-items-center rounded-2xl text-xl shadow-sm"
            ...
            @switch (icon) {
              ...
              @case ('globe') { <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M3.6 9h16.8M3.6 15h16.8M12 3a15 15 0 010 18 15 15 0 010-18z"/></svg> }
              @default { <span class="text-base font-bold">{{ icon }}</span> }
            }
          </span>
          <div class="min-w-0 flex-1">
            <h2 class="sheet-title truncate text-lg font-extrabold">{{ title }}</h2>
            @if (subtitle) {
              <p class="sheet-sub truncate text-[13px] font-medium">{{ subtitle }}</p>
            }
          </div>
          <button type="button" (click)="close.emit()"
                  class="sheet-close grid h-10 w-10 place-items-center rounded-2xl active:scale-95"
                  aria-label="Cerrar">
